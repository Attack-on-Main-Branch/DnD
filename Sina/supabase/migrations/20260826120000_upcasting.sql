-- "Frieren cast Fireball at 5th Level (10d6 Fire / DC 15 DEX)."
--
-- Upcasting, in the two places it shows: what a spell throws when it is cast
-- from a bigger slot, and what the table is told about it afterwards.
--
-- The scaling tables come first. The SRD tabulates a spell's dice per slot
-- level rather than describing them -- `damage_at_slot_level` for a Fireball,
-- `heal_at_slot_level` for a Cure Wounds, `damage_at_character_level` for a
-- cantrip that grows with its caster -- and without them an upcast could only
-- roll the base dice and print a paragraph of prose about the rest.
--
-- jsonb and not two more text columns: it is a map of level to notation, at
-- most twenty entries, and it is only ever read whole.

alter table public.character_spells
  add column if not exists damage_by_level jsonb not null default '{}'::jsonb,
  add column if not exists heal_by_level jsonb not null default '{}'::jsonb;

do $ck$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'character_spells_scaling_check'
  ) then
    alter table public.character_spells
      add constraint character_spells_scaling_check
      check (
        jsonb_typeof(damage_by_level) = 'object'
        and jsonb_typeof(heal_by_level) = 'object'
      );
  end if;
end;
$ck$;
...
-- ---------------------------------------------------------------------------
-- What the log may now say about a spell.
-- ---------------------------------------------------------------------------
--
-- Two more facts on the entry 20260825120000 added: the dice and the save. Both
-- are DATA and not a sentence -- they are copied off the spell's own row, and
-- Maria's activity-presentation.jsx is still where the English is written.
--
-- No change to the action list, so no constraint to drop: `spell_cast` has been
-- admitted since 20260825120000 and this only widens what rides with it.
--
-- The whole of that file's function again with the branch widened, not a patch
-- over it: several files here `create or replace` the same function, so this is
-- now the highest-numbered one that touches it and the one to re-run after any
-- out-of-order paste.
--
...
-- the fifteen-argument version would go on answering for anyone still calling
-- it, silently writing no dice.
...
revoke all on function public.record_campaign_activity(uuid, uuid, text, uuid, text, integer, text, integer, integer, integer, integer, text, integer, text, integer, text, text) from public;
revoke all on function public.record_campaign_activity(uuid, uuid, text, uuid, text, integer, text, integer, integer, integer, integer, text, integer, text, integer, text, text) from anon;
grant execute on function public.record_campaign_activity(uuid, uuid, text, uuid, text, integer, text, integer, integer, integer, integer, text, integer, text, integer, text, text) to authenticated;
