-- "Dungeon Master took 50 Gold from the party."
--
-- 20260823180000 would not write that line. It admitted a null recipient for
-- one action only -- `coin_granted` -- on the argument the item log makes:
-- "Nothing is ever taken back from the party: the stepper that revokes works
-- one purse at a time." That stepper is gone. `move_campaign_currency` takes
-- five denominations off every purse at the table in one transaction, so the
-- sentence describing it now exists and has to be writable.
--
-- ITEMS ARE UNCHANGED, and deliberately so. `item_revoked` still refuses a null
-- recipient, because taking an item back really is still one pack at a time.
-- The two lists part company here for the first time.
--
-- The whole of 20260823180000's function again with that one condition widened,
...
--
-- No `drop` first, unlike 20260823120000 and 20260823180000: the signature does
-- not change, so there is no second door for PostgREST to go on answering.
