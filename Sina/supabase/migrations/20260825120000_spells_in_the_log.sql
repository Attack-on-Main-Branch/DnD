...
--
-- One entry, and it is the one thing that can happen to a spell in front of the
-- party. Learning one is bookkeeping and preparing one is a morning, but a
-- spell cast is a thing everybody at the table saw -- which is the line the
-- log has drawn since 20260823090000, and the same reason `item_used` is in it
-- while a pack being opened is not.
--
-- The level rides along because it is not derivable from the name: a Magic
-- Missile at third is a different event from one at first, and the shelf the
-- spell sits on is what the table wants told.
--
...
-- any database that already had one -- refusing `spell_cast` with nothing to
-- say why.
...
-- ---------------------------------------------------------------------------
--
-- The whole of 20260824120000's function again with a spell branch added, not a
...
-- the thirteen-argument version would go on answering for anyone still calling
-- it, silently writing no spell.
...
revoke all on function public.record_campaign_activity(uuid, uuid, text, uuid, text, integer, text, integer, integer, integer, integer, text, integer, text, integer) from public;
revoke all on function public.record_campaign_activity(uuid, uuid, text, uuid, text, integer, text, integer, integer, integer, integer, text, integer, text, integer) from anon;
grant execute on function public.record_campaign_activity(uuid, uuid, text, uuid, text, integer, text, integer, integer, integer, integer, text, integer, text, integer) to authenticated;
