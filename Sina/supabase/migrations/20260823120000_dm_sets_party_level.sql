-- The Dungeon Master's pen over the party's levels, and the log entry it leaves.
--
-- 20260821160000 gave hit points the same shape, and this is its mirror rather
-- than its copy: damage is called out by whoever runs the session, so health is
-- writable by the character's owner AND by the Dungeon Master. A LEVEL is
-- awarded, so this one is the head of the table's alone.
--
-- Scoped by campaign for the reason that file gives: a character can sit at more
-- than one table, so "is a Dungeon Master" is not the question, and membership
...
revoke all on function public.set_character_level(uuid, integer, uuid) from public;
revoke all on function public.set_character_level(uuid, integer, uuid) from anon;
grant execute on function public.set_character_level(uuid, integer, uuid) to authenticated;

-- ---------------------------------------------------------------------------
-- What the log may now say.
-- ---------------------------------------------------------------------------
--
...
-- any database that already had one -- refusing `level_change` with nothing to
-- say why.
...
-- ---------------------------------------------------------------------------
--
-- The whole of 20260823090000's function again with a `level_change` branch
-- added, not a patch over it: several files here `create or replace` the same
...
-- set of keys it is handed -- so the nine-argument version would go on
-- answering for anyone still calling it, silently writing no level.
--
-- `level_value` and `level_delta` are both passed in rather than re-derived,
-- which is the one place this function departs from its own rule. Re-reading
-- `characters.level` would be worse: the level is written before the entry
-- describing it, so two quick presses would both find whatever the second one
-- wrote and the log would say "to 6" twice. Both are bounded below.
...
revoke all on function public.record_campaign_activity(uuid, uuid, text, uuid, text, integer, text, integer, integer, integer, integer) from public;
revoke all on function public.record_campaign_activity(uuid, uuid, text, uuid, text, integer, text, integer, integer, integer, integer) from anon;
grant execute on function public.record_campaign_activity(uuid, uuid, text, uuid, text, integer, text, integer, integer, integer, integer) to authenticated;
