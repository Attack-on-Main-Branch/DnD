...
export function createBrush() {
  const stamps = []
  for (let i = 0; i < BUCKETS; i++) stamps.push(makeStamp(sampleRamp(i / (BUCKETS - 1))))
  ...
  return function stampSegment(ctx, segment, spread, brightness) {
    ...
    const steps = Math.min(10, Math.max(1, Math.ceil(segment.len / (radius * 0.3))))
    const spacing = segment.len / steps
    const alpha = Math.min(1, (brightness * spacing) / (radius * K))
    if (alpha <= 0.0015) return

    const stamp = stamps[Math.min(BUCKETS - 1, Math.max(0, Math.round(segment.t * (BUCKETS - 1))))]
    const size = radius * 2
